...
namespace RoninEngine::UI {
   ...
   bool CText::render_control(GUI* gui, RenderData& data, SDL_Renderer* render,
                              bool* hover) {
      Render_String(render, data.rect, data.text.c_str(), data.text.size());
      return false;
   }

   bool CButton::render_control(GUI* gui, RenderData& data, SDL_Renderer* render,
                                bool* hover) {
      ...
      *hover = SDL_PointInRect((SDL_Point*)&ms, (SDL_Rect*)&data.rect);
      ...
      // fill
      guiInstance->GUI_SetMainColorRGB(
               *hover ? input::isMouseDown() ? 0xab0000 : 0xe70000 : 0xc90000);
      rect = data.rect;
      rect += inside / 2;
      SDL_RenderFillRect(render, (SDL_Rect*)&rect);

      // render text
      Render_String(render, data.rect, data.text.c_str(), data.text.size(), 13,
                    TextAlign::MiddleCenter, true, *hover);
      bool msClick = input::isMouseUp();
      return (*hover) && msClick;
   }

   bool CEdit::render_control(GUI* gui, RenderData& data, SDL_Renderer* render,
                              bool* hover) {
      return false;
   }

   bool CHorizontalSlider::render_control(GUI* gui, RenderData& data,
   ...
   bool CVerticalSlider::render_control(GUI* gui, RenderData& data,
                                        SDL_Renderer* render, bool* hover) {
      return false;
   }
   ...
   bool CImageAnimator::render_control(GUI* gui, RenderData& data,
                                       SDL_Renderer* render, bool* hover) {
   ...
   bool CTextRandomizerDisplay::render_control(GUI* gui, RenderData& data,
                                               SDL_Renderer* render, bool* hover) {
      static float lasttime = 0;

      // todo: Доработать рандомайзера!
      TextRandomizer_Format format =
            TextRandomizer_Format::OnlyNumber;  // (int)(data.resources);
