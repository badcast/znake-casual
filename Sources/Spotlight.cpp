...
Spotlight::Spotlight(const string &name) : Light(name) {
  ...
  if (!spotlightMask) {
    spotlightMask =
        GC::resource_bitmap("spotlight", FolderKind::TEXTURES);
  }
}
...
void Spotlight::GetLight(SDL_Renderer *renderer) {
  ...
  SDL_QueryTexture(texture, nullptr, reinterpret_cast<int *>(&access), nullptr,
                   nullptr);

  SDL_RenderCopyEx(renderer, texture, nullptr, nullptr, transform()->angle(),
