...
const Vec2 Vec2::NAbs(const Vec2& value) {
    return {Mathf::nabs(value.x), Mathf::nabs(value.y)};
}
...
Vec2 Vec2::MoveTowards(const Vec2& current, const Vec2& target,
...
Vec2 Vec2::SmoothDamp(Vec2 current, Vec2 target, Vec2& currentVelocity,
...
bool Vec2::AreaIntersectRect(const SDL_FRect& A, const SDL_FRect& B,
                             SDL_FRect& result) {
...
void Vec2::Area_UnionRect(const SDL_Rect* A, const SDL_Rect* B,
                          SDL_Rect* result) {
    int Amin, Amax, Bmin, Bmax;
...
bool AreaEnclosePoints(const Rectf_t* points, int count, const Rectf_t* clip,
                       Rectf_t* result) {
    float minx = 0;
    float miny = 0;
    float maxx = 0;
    ...
    if (clip) {
        ...
        for (i = 0; i < count; ++i) {
            ...
            if (x < clip_minx || x > clip_maxx || y < clip_miny ||
                y > clip_maxy) {
                continue;
            }
            if (!added) {
...
bool Vec2::InArea(const Vec2& p, const SDL_FRect& r) {
...
Vec2 RoninEngine::operator+(const Vec2& lhs, const Vec2& rhs) {
...
bool RoninEngine::operator==(const Vec2& lhs, const Vec2& rhs) {
...
Vec2 RoninEngine::operator*(const float& d, const Vec2& rhs) {
...
Vec2 RoninEngine::operator*(const Vec2& rhs, const float& d) { return d * rhs; }
