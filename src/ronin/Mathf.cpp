...
namespace RoninEngine {
	...
	float Mathf::Clamp01(float val) {
		return Clamp(val, 0, 1.f);
	}

	float Mathf::Clamp(float val, const float min, const float max) {
	...
	float Mathf::nabs(float x) {
		return abs(x) * ~0;
	}

	//negative absolute value
	int Mathf::nabs(int x) {
		return abs(x) * ~0;
	}
